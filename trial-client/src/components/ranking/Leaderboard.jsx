import { Flame, Crown, UserPlus, UserCheck } from "lucide-react";
import Avatar from "../ui/Avatar";
import { useI18n } from "../../i18n/LanguageContext";

const MEDAL = ["#f6b100", "#b8bcc4", "#cd7f4b"];

export default function Leaderboard({ rows, following, onToggleFollow }) {
  const { t } = useI18n();
  return (
      ...
      {rows.map((r, i) => {
        ...
        return (
          ...
          >
            ...
            {!r.isYou && onToggleFollow && (
              <button
                onClick={() => onToggleFollow(r.id)}
                aria-pressed={isFollowing}
                aria-label={isFollowing ? t("profile.following") : t("profile.follow")}
                className={`glow inline-flex h-8 shrink-0 items-center gap-1 rounded-full px-2.5 text-[0.76rem] font-semibold ${
                  isFollowing ? "bg-accent-surface text-accent-strong" : "border border-line-strong text-ink-2 hover:bg-sunken"
                }`}
              >
                {isFollowing ? <UserCheck className="h-3.5 w-3.5" /> : <UserPlus className="h-3.5 w-3.5" />}
                <span className="hidden sm:inline">{isFollowing ? t("profile.following") : t("profile.follow")}</span>
              </button>
            )}
